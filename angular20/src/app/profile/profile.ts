...
export class Profile implements OnInit  {  
  ...
  ) 
  {
    ...
    this.profileService.getUserbyId(this.userId, this.jwttoken).subscribe({
     next: (res: any) => {
           this.profileMsg.set(res[0].message);
           const formData = {
            ...
            this.profilepic = res[0].userpicture;
            if (res[0].qrcodeurl !== null) {
              let qrcode: any = 'data:image/png;base64,'+res[0].qrcodeurl;
              this.qrcodeurl = qrcode;  
              this.mfa = true;
            } else {
             this.mfa = false;
              this.qrcodeurl = "http://127.0.0.1:5000/assets/images/qrcode.png";
            }
            setTimeout(() => {
              this.profileMsg.set('');
            }, 1000);
